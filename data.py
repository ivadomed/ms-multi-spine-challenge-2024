...
def convert_to_nnUNet_format(root_dir, output_dir, task_name="Dataset025_ms-challenge"):
    ...
    for sub in subjects:
        ...
        t2w_image = os.path.join(anat_dir, f"{sub}_T2w.nii.gz")
        label_image = os.path.join(label_dir, f"{sub}_T2w_label-lesion_seg.nii.gz")
        
        if os.path.exists(t2w_image):
