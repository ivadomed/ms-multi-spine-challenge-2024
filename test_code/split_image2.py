...
dataset_split_root = "dataset_split"
...
def process_split(split_folder, split_key):
    split_dir = os.path.join(dataset_split_root, split_folder)
    if not os.path.exists(split_dir):
        print(f"Warning: {split_folder} folder not found in {dataset_split_root}")
        return
    ...
    for subject_name in os.listdir(split_dir):
        ...
        seg_file = None
        for file in os.listdir(subject_path):
            if file.endswith(".nii.gz") and "label-lesion_seg" in file:
                seg_file = os.path.join(subject_path, file)
                break
...
split_json_path = os.path.join(dataset_split_root, "dataset_split.json")
