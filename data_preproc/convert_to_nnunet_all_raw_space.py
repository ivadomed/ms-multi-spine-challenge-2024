...
More precisely, it creates the dataset 21, 22, 23, 24 and 25.
Datasets:
- 21: T2w raw space cropped masked around the spinal cord
- 22: PSIR raw space cropped masked around the spinal cord
- 23: STIR raw space cropped masked around the spinal cord
- 24: MP2RAGE raw space cropped masked around the spinal cord
- 25: all raw space cropped masked around the spinal cord
...
def agg_data(data_dir, dataset_type):
    ...
    """
    images = []
    if dataset_type == 25: # dataset 25 (monomodal_all_preprocReg_T2wspace)
        # List all preproc
    ...
    elif dataset_type == 21: # dataset 21 (monomodal_T2w_preproc_T2wRawSpace)
    ...
    elif dataset_type == 22: # dataset 22 (monomodal_PSIR_preproc_T2wspace)
    ...
    elif dataset_type == 23: # dataset 23 (monomodal_STIR_preproc_T2wspace)
    ...
    elif dataset_type == 24: # dataset 24 (monomodal_MP2RAGE_preproc_T2wspace)
...
def convert_to_nnUNet_format(agg_data, output_dir, path_data_split, task_name, task_number, dataset_type):
    ...
    for file in agg_data:
        ...
        if contrast == 'T2w':
            # 1. If the image is a T2w image then we juste have to move it back to its original space
            assert os.system(f"sct_register_multimodal -i {file} -d {t2w_raw_image} -identity 1 -ofolder {temp_folder} -owarp {temp_folder/'warp_image_to_t2wraw.nii.gz'} -o {temp_folder/'image_reg_to_t2wraw.nii.gz'} -qc {path_registration_qc} -dseg {temp_folder/'file_raw_image_sc_seg.nii.gz'} -qc-subject {file.split('/')[-1]}") == 0
            # 2. Label file in raw T2w space does not need to be edited
            # 3. Copy the registered file to the nnunet folder
            assert os.system(f"cp {temp_folder/'image_reg_to_t2wraw.nii.gz'} {out_image}") == 0
            # 4. Copy the label file to the nnunet folder
            assert os.system(f"cp {label_file} {out_label}") == 0
        # If other contrast: we register the T2w raw image to the corresponding file then apply the warping field to the T2w raw segmentation
            ...
            assert os.system(f"cp {temp_folder/'image_reg_to_image_raw.nii.gz'} {out_image}") == 0        
