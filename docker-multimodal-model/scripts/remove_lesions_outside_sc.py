...
def remove_lesions_outside_sc(subj_dict, output_folder):
    ...
    pred_segmentations = [subj_dict['segmentation_file'], subj_dict['soft_mask']]
    ...
    t2w_raw_image_orientation = Image(subj_dict['rawdata_T2']).orientation
    ...
    resolution_r_l = get_dimension(Image(subj_dict['rawdata_T2']))[4+ r_l_direction]
    vox_dilate_r_l = max(1,int(2 / resolution_r_l))  # 2mm dilation in the R-L axis

    # same for the A-P axis
    resolution_a_p = get_dimension(Image(subj_dict['rawdata_T2']))[4+ a_p_direction]
    ...
    subj_dict['t2_segmentation_file_rmv_lesions_outside_sc'] = os.path.join(output_folder, Path(subj_dict["segmentation_file"]).name.replace('.nii.gz', '_rmv_lesions_outside_sc.nii.gz'))
